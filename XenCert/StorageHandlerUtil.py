...
def InitLogging():
    ...
    logfilename = os.path.join(
        '/tmp', 'XenCert-' + commands.getoutput('uuidgen') + '.log')
...
def displayOperationStatus(passOrFail, customValue=''):
    if passOrFail:
        Print(
            "                                                                                                   PASS [Completed%s]" % customValue)
    else:
        Print("                                                                                                   FAIL [%s]" % time.asctime(
            time.localtime()))
...
def IsMPEnabled(session, host_ref):
    try:
        ...
        XenCertPrint(
            "Exception determining multipath status. Exception: %s" % str(e))
    return False
...
def disable_multipathing(session, host):
    try:
        ...
        XenCertPrint(
            "Exception disabling multipathing. Exception: %s" % str(e))
...
def blockIP(ip):
    try:
        ...
        XenCertPrint(
            "There was an exception in blocking ip: %s. Exception: %s" % (ip, str(e)))
...
def unblockIP(ip):
    try:
        ...
        XenCertPrint(
            "There was an exception in unblocking ip: %s. Exception: %s" % (ip, str(e)))


def actualSRFreeSpace(size):
    num = (size - lvutil.LVM_SIZE_INCREMENT - 4096 -
           vhdutil.calcOverheadEmpty(MSIZE)) * vhdutil.VHD_BLOCK_SIZE
...
def GetConfig(scsiid):
    try:
        ...
        cmd = ["/usr/lib/udev/scsi_id", "--replace-whitespace",
               "--whitelisted", "--export", device]
        ret = util.pread2(cmd)
        ...
        XenCertPrint(
            "There was an exception getting SCSI device config. Exception: %s" % str(e))
        retVal = False
...
def findIPAddress(mapIPToHost, HBTL):
    try:
        ...
        XenCertPrint(
            "There was an exception in finding IP address for the HBTL: %s. Exception: %s" % (HBTL, str(e)))
    return ip

# The returned structure are a list of portals, and a list of SCSIIds for
# the specified IQN.
...
def GetListPortalScsiIdForIqn(session, server, targetIqn, chapUser=None, chapPassword=None):
    try:
        ...
        try:
            session.xenapi.SR.probe(util.get_localhost_uuid(
                session), device_config, 'lvmoiscsi')
        except Exception, e:
        ...
        try:
            ...
            XenCertPrint(
                "The portal list at the end of the iteration is: %s" % listPortal)
        except Exception, e:
            raise Exception(
                "The target %s did not return any IQNs on probe. Exception: %s" % (server, str(e)))
        ...
        for iqn in targetIqn.split(','):
            ...
            try:
                ...
                XenCertPrint(
                    "The IQN: %s did not return any SCSI IDs on probe. Exception: %s" % (iqn, str(e)))

            XenCertPrint("Got the SCSIId list for iqn %s as %s" %
                         (iqn, listSCSIId))

    except Exception, e:
        XenCertPrint(
            "There was an exception in GetListPortalScsiIdForIqn. Exception: %s" % str(e))
        raise Exception(str(e))

    XenCertPrint(
        "GetListPortalScsiIdForIqn - returning PortalList: %s." % listPortal)
    XenCertPrint(
        "GetListPortalScsiIdForIqn - returning SCSIIdList: %s." % listSCSIId)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                XenCertPrint(
                    "The HBA information list being returned is: %s" % list)
            except Exception, e:
                XenCertPrint(
                    "Failed to parse lvmohba probe xml. Exception: %s" % str(e))

    except Exception, e:
        XenCertPrint(
            "There was an exception in GetHBAInformation: %s." % str(e))
        Print("Exception: %s" % str(e))
        retVal = False

    XenCertPrint(
        "GetHBAInformation - returning adapter list: %s and scsi id list: %s." % (list, scsiIdList))
    return (retVal, list, scsiIdList)

# the following details from the file name, put it into a list and return
# the list.
...
def GetLunInformation(id):
    ...
    listLunInfo = []
    try:
        # take in a host id, then list all files in /dev/disk/by_scsibus of the
        # form *-5* then extract
...
def PlugAndUnplugPBDs(session, sr_ref, count):
    PrintOnSameLine(
        "      Unplugging and plugging PBDs over %d iterations. Iteration number: " % count)
    try:
        checkPoint = 0
...
def FindTimeToWriteData(devicename, sizeInMiB):
    ...
    XenCertPrint(
        "Now copy %dMiB data from /dev/zero to this device and record the time taken to copy it." % sizeInMiB)
    cmd = ['dd', 'if=/dev/zero', ddOutFile,
           'bs=4096', 'count=%d' % (sizeInMiB * 256)]
    try:
...
def get_lun_scsiid_devicename_mapping(targetIQN, portal):
    ...
    try:
        ...
        XenCertPrint(
            "Failed to find any LUNs for IQN: %s and portal: %s" % (targetIQN, portal))
        return {}
...
def parse_config(vendor, product):
    try:
        ...
        XenCertPrint("Failed to get multipath config for vendor: %s and product: %s. Exception: %s" % (
...
def parse_xml_config(file):
    ...
    optional = ['port', 'protocol', 'chapuser',
                'chappass', 'lunsize', 'growsize']
    for val in required + optional:
        try:
            configuration[val] = str(config_info.getElementsByTagName(val)[
...
def get_path_status(scsi_id, onlyActive=False):
    ...
    try:
        ...
        XenCertPrint("There was some exception in getting path status for scsi id: %s. Exception: %s" % (
            scsi_id, str(e)))
        retVal = False
...
def FindDiskDataTestEstimate(device, size):
    ...
    XenCertPrint(
        "Run diskdatatest in a report mode with device %s to find the estimated time." % device)
    ...
    XenCertPrint("Total estimated time for testing IO with the device %s as %d" % (
...
def _find_LUN(svid):
    ...
    if svid.startswith("NETAPP_"):
        ...
        globstr = basepath + "NETAPP__LUN__" + "*" + \
            svid_parts[2] + "*" + svid_parts[-1] + "*"
    else:
